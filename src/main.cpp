...
void pre_auton(void) {
 ...
 Controller_Reset();
 Controller1.Screen.print("Comp-Blue");
...
//Blake==THE BIG DUMB
 ...
 if(Potentiometer.angle(degrees) >= 186.5 && Potentiometer.angle(degrees) >= 10) //right  still in progress //range = Min=0.6187 Max=249.94
 {
RightIntake.stop(coast);
  LeftIntake.stop(coast);
  RampMotor.stop(coast);
...
RightIntake.setVelocity(100,percent);
  ...
  Right_Turn(37,-90);//25
 Drivetrain.setDriveVelocity(55,percent);//10
 Drivetrain.driveFor(forward,5,inches);

 Drivetrain.setDriveVelocity(50,percent);//45
 ...
 //Stacking
 LeftIntake.spinFor(reverse,.13*5,turns,false);
  RightIntake.spinFor(reverse,.13*5,turns,false);//4
 ...
 else if(Potentiometer.angle(degrees) <= 186.5 && Potentiometer.angle(degrees) >= 10) //left (stacks 4)
 {
   //Arm_Movement(30, 200);
  Intake_In(100,3);
  ...
  LeftIntake.setVelocity(20,percent);
  RightIntake.setVelocity(20,percent);
  ...
  Ramp_Movement(31,115);//125 after testing
...
void usercontrol(void) {
 ...
 while (1) {
   ...
   if(Controller1.ButtonA.pressing()){
     if(RampMotor.position(degrees) >= 777*7)
     {//www.blake==dumn.net
  ...
  //blake says that you are very warm
 ...
 // you're a cutie
   ...
   if(Brain.Battery.capacity() == Low_Battery_Amount ){
